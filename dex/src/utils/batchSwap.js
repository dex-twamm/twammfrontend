...
export async function runQueryBatchSwap(assetInAddress, assetOutAddress, swapAmount) {
    let spotPrice, expectedSwapOut = 0;
    let errorMessage = "";
    const assetInIndex = POOLS[POOL_ID].tokens.findIndex(object =>
        assetInAddress === object.address
    );
    const assetOutIndex = POOLS[POOL_ID].tokens.findIndex(object =>
        assetOutAddress === object.address
    );

    console.log("ShortSwap Address", assetInAddress, assetInIndex, assetOutIndex, swapAmount);
    try {
        const config = {
            network: 5,
