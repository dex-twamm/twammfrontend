...
const Swap = props => {
	...
	const handleChange = (e, newValue) => {
		if (typeof newValue === 'number') {
			...
			setSliderValueUnit(
				valueLabel(calculateValue(newValue)).sliderUnits
			);
		...
		<>
			<form onSubmit={handleSubmit} className={styles.form}>
				...
				{swapType !== 'long' && (
						<>
							<FontAwesomeIcon
								className={classNames(style.iconDown,
									swapAmount !== 0 && tokenB.tokenIsSet && swapType !== 'long' && style.openIconDown)}
								icon={faArrowDown}
							/>
							<Input
								id={2}
					...
					<Box sx={{
						...
						<InfoOutlinedIcon 
						 	sx={{color:'#808080', cursor:'pointer',fontSize:'20px' ,display:{xs:'none',sm:'block'} }}  
							 />
								<span style={{
								...
								<span style={{color:'#333333',opacity:0.7}}> ($123)</span>
								 </span>
						...
						<Box sx={{
							...
							<span style={{color:'#333333',opacity:0.7,background:'#f7f8fa',borderRadius:'10px' ,padding:'4px 6px'}}>$1.23</span>
								...
								<KeyboardArrowUpOutlinedIcon
									sx={{fontSize:'24px',color:'#333333',cursor:'pointer'}}
									onClick={handleClose}
